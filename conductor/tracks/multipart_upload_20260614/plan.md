...
- [ ] Task: Update backend unit tests to request multipart/form-data
    - [ ] Modify tests in src/test/api/workspace/[customer_slug]/files/route.test.ts to use FormData payload mocks
- [ ] Task: Refactor workspace files POST handler to parse FormData
    - [ ] Implement FormData parsing using req.formData() in POST handler in src/app/api/workspace/[customer_slug]/files/route.ts
    - [ ] Verify validation and type checking on uploaded files
- [ ] Task: Refactor programmatic v1 upload endpoint to parse FormData
    - [ ] Implement FormData parsing using req.formData() in POST handler in src/app/api/v1/files/route.ts
- [ ] Task: Conductor - User Manual Verification 'Test Updates & Backend Endpoint Refactoring' (Protocol in workflow.md)
